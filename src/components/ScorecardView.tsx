import type { Scorecard, MetricScore, BizScore } from "@/lib/scorecard";
import { ScoreRings } from "@/components/ScoreRings";
import { DraftButton } from "@/components/DraftButton";

/**
 * The "You vs Your Market" scorecard body — rings hero + gap-to-best table + bullet
 * tiles + heatmap + action plan. Shared by the in-app /scorecard page and the public,
 * claimable free-read at /read/[token]. `publicView` drops the interactive Draft
 * buttons (no auth on the public page) and shows the moves as a preview.
 */
...
function ActionPlan({ metrics, publicView }: { metrics: MetricScore[]; publicView?: boolean }) {
      ...
      <div className="space-y-2.5">
        {behind.map((m, i) => { const mv = MOVE[m.key](m.avg!); return (
          <div key={i} className="flex flex-wrap items-start gap-x-3 gap-y-2 rounded-2xl bg-white/55 p-3.5">
            ...
            {!publicView && <DraftButton move={`${mv.title}: ${mv.why}`} context={mv.context} />}
          </div>
        ); })}
      </div>
    </section>
...
export function ScorecardView({ sc, businessName, publicView }: { sc: Scorecard; businessName: string; publicView?: boolean }) {
  return (
    <div className="space-y-6">
      <section className="card">
        <div className="grid gap-6 md:grid-cols-[300px_1fr] md:items-center">
          ...
          <div>
            {sc.headline && <p className="mb-3 text-sm"><span className="font-semibold text-ink">{publicView ? `${businessName} vs the market:` : "Rani's read:"}</span> <span className="text-ink-soft">{sc.headline}</span></p>}
            <div className="overflow-x-auto">
      ...
      <section className="card">
        ...
        <p className="mb-3 text-xs text-ink-faint">Every business across every metric — {publicView ? "you" : "you"} highlighted, darker = stronger.</p>
        <Heatmap businesses={sc.businesses} metrics={sc.metrics} />
      </section>

      <ActionPlan metrics={sc.metrics} publicView={publicView} />
    </div>
  );
}
