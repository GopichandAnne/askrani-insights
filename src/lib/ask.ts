...
const GROUNDING = [
  ...
  "For DENTAL procedure-cost questions ('how much is a crown / root canal / implant near me?'), use DATA.dentalBenchmark — an area ballpark per procedure (a standardized-code benchmark scaled to the region), plus any price actually seen locally. Prefer a PUBLISHED local price (DATA.intel.priceAnchors — a clinic's own posted fee) over a benchmark estimate; give the area range, name the locally-published figure when present, and always say it's an estimate — the exact fee depends on the tooth/materials/insurance.",
  "For DENTAL pricing-transparency questions or positioning advice, use DATA.intel.priceAnchors — how many local competitors publish real prices vs keep them off their site, and whether YOU publish. Most private practices hide pricing, so if the owner doesn't publish, transparent pricing (or a clear new-patient special) is a concrete way to stand out; if rivals publish aggressive specials, flag them.",
  "For DENTAL insurance questions ('do I take the right plans?', 'what insurance do competitors accept?'), use DATA.intel.insurance — which plans each practice lists, the plans competitors accept that YOU don't advertise (a booking gap to close if you're in-network), and any plan only you take (a differentiator to promote). Insurance acceptance is a top patient filter, so treat gaps as high-priority.",
...
export function makeIntel(scorecard: Scorecard, goals: Record<string, any>): Record<string, unknown> {
  ...
  return {
    ...
    priceAnchors: g.priceAnchors && !g.priceAnchors.empty ? { summary: clip(g.priceAnchors.summary, 200), publishedCompetitors: g.priceAnchors.publishedCount, opaqueCompetitors: g.priceAnchors.opaqueCount, youPublish: g.priceAnchors.youPublish, publishedPrices: arr(g.priceAnchors.anchors).slice(0, 10).map((a: any) => ({ procedure: a.label, low: a.low, high: a.high, from: arr(a.prices).slice(0, 3).map((p: any) => ({ business: p.business, price: p.amount, type: p.type })) })) } : null,
